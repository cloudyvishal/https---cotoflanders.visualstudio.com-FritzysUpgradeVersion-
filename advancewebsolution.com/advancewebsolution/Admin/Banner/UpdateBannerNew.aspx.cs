...
{
    public partial class Admin_Baner_UpdateCoupon : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (fup.HasFile == true)
                {
                    string ImagePath = fup.FileName;
                    fup.SaveAs(Server.MapPath("mobileimages/" + ImagePath + ""));

                    string ImagePath1 = ("mobileimages/" + ImagePath + "");
                    Banner objBanner = new Banner();
                    objBanner.InsertImage(ImagePath1);
                }
            }
            catch (Exception Ex) { throw Ex; }
        }

        protected void drpcouponselect_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (drpcouponselect.SelectedValue == "1")
            {
                tdmain.Visible = true;
            }
        }
