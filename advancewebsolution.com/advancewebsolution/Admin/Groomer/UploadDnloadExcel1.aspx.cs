...
namespace advancewebtosolution
{
    public partial class Admin_Groomer_UploadDnloadExcel : System.Web.UI.Page
    {
        ...
        string strGId, CustomerName, Job, ZipCode, Pets, Rebook, FromRebook, New, TimeIn, TimeOut, PetTime, ExtraServices, CreditCardNo, CreditCardExpir, CreditCardORChkName, CreditCardORChkAddr, SecurityCode, NextAppomentDate, NextAppomentTime, NextAppointmentEndTime, ServicesForPet1, ServicesForPet2, ServicesForPet3, ServicesForPet4, ServicesForPet5, ServicesForPet6;
        int AppointmentID;
        string revValidate = "";
        ...
        Excel.Worksheet excelSheetInv = new Microsoft.Office.Interop.Excel.Worksheet();
        ...
        private Excel.Application excelAppEnd = new Excel.ApplicationClass();
        object missingEnd = System.Reflection.Missing.Value;
        Excel.Worksheet excelSheetEnd = new Microsoft.Office.Interop.Excel.Worksheet();
            ...
            if (FileUpload1.PostedFile.FileName != "")
            {
                if (fn.Contains(".xls"))
                {
                    ...
                    if (FileName == true)
                    {
                        ...
                        Response.Redirect("UploadDnloadExcel.aspx?Msg=0");
                    }
                    else
                    {
                        FileUpload1.PostedFile.SaveAs(SaveLocation);
                                            ...
                                            for (int k = secRow; k < (secRow + 10); k++)
                                            {
                                                ...
                                                if (range != null)
                                                {
                                                    if (excelSheet.get_Range("A" + k, "A" + k).Text.ToString() == string.Empty)
                                                    {
                                                        ...
                                                        //excelSheet.get_Range("U" + k, "U" + k).Value2 = CreditCardNumber_data;
        ...
        public void getDailyOperationLog(int m, DataSet ds)
        {
            ...
            if (dsNoExp.Tables[0].Rows.Count > 0)
            {
                ...
                FuelPurchased_Data = dsNoExp.Tables[0].Rows[m]["FuelPurchased"].ToString();
                PricePerGallon_Data = dsNoExp.Tables[0].Rows[m]["PricePerGallon"].ToString();
                ...
                CreditCardNo_Data = dsNoExp.Tables[0].Rows[m]["CreditCardNo"].ToString(); //only last three no
                DateTime dt1 = Convert.ToDateTime(dsNoExp.Tables[0].Rows[m]["CreditCardExpir"].ToString());
                CreditCardExpir_Data = dt1.ToShortDateString();
                CreditCardName_Data = dsNoExp.Tables[0].Rows[m]["CreditCardORChkName"].ToString();
                CreditCardAddr_Data = dsNoExp.Tables[0].Rows[m]["CreditCardORChkAddr"].ToString();
                SecurityCode_Data = dsNoExp.Tables[0].Rows[m]["SecurityCode"].ToString();
                //Bind all data in one filed 
                if (CreditCardExpir_Data == "1/1/1900")
                    CreditCardNo_Data = "";
                else
                    CreditCardNo_Data = CreditCardNo_Data + ", " + CreditCardExpir_Data + ", " + CreditCardName_Data + ", " + CreditCardAddr_Data + ", " + SecurityCode_Data;
                string creditcardnumberch = dsNoExp.Tables[0].Rows[m]["CreditCardNo"].ToString();
                //if (dsNoExp.Tables[0].Rows[m]["CreditCardNo"]!= "")// || dsNoExp.Tables[0].Rows[m]["CreditCardNo"]!= "ccy" || dsNoExp.Tables[0].Rows[m]["CreditCardNo"]!=null)
                //{
        ...
        public void ExportToExcelNewInventroy()
        {
            ...
            if (dsNoExpInv.Tables[0].Rows.Count > 0)
            {
                ...
                Excel.Workbook excelWorkBook = excelAppInv.Workbooks.Open(ExistsExcelFilePath1, 0, false, 5, "albhistar1", "albhistar1", true, Microsoft.Office.Interop.Excel.XlPlatform.xlWindows, "\t", true, false, 0, true, 1, 0);
                Excel.Range range = null;
                NextRow:
        ...
        public void ExportToExcelEndingMileage()
        {
            ...
            if (dsNoExpEnd.Tables[0].Rows.Count > 0)
            {
                ...
                Excel.Workbook excelWorkBook = excelAppEnd.Workbooks.Open(ExistsExcelFilePath1, 0, false, 5, "albhistar1", "albhistar1", true, Microsoft.Office.Interop.Excel.XlPlatform.xlWindows, "\t", true, false, 0, true, 1, 0);
                Excel.Range range = null;
                NextRow:
        ...
        protected void Push_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("UploadDnloadExcel.aspx?Msg=1");
        }
    }
}