...
public partial class PaymentPrepaid : Page
{
    #region var
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            if (!(null == Session["UserName"]) && !(null == Session["UserID"]))
            {
                ...
                else BindAppointments();
                StoreFrontUser ObjUser = new StoreFrontUser();
                DataSet ds = ObjUser.GetUser(Convert.ToInt32(Session["UserID"].ToString()));
    ...
    public void BindAppointments()
    {
        ...
        ds = obj.GetPaymentFutureAppointment(Convert.ToInt32(Session["UserID"].ToString()), Session["UserName"].ToString());
        if (ds.Tables[0].Rows.Count > 0)
        {
    ...
    #region Generate Reference No Randomaly
    ...
    protected void configure()
    {
        ...
        //config.ConnectionLimit = int.Parse(ConfigurationManager.AppSettings["cybs.connectionLimit"].ToString());
        //config.EnableLog =  bool.Parse(ConfigurationManager.AppSettings["cybs.enableLog"].ToString());
        config.KeyFilename = ConfigurationManager.AppSettings["cybs.keyFilename"].ToString();
        config.KeysDirectory = Server.MapPath("~") + "Keys";
        //config.LogDirectory =ConfigurationManager.AppSettings["cybs.logDirectory"].ToString();
        //config.LogFilename = ConfigurationManager.AppSettings["cybs.logFilename"].ToString();
        //config.LogMaximumSize = int.Parse(ConfigurationManager.AppSettings["cybs.logMaximumSize"].ToString());
        config.MerchantID = ConfigurationManager.AppSettings["cybs.merchantID"].ToString();
        //config.SendToProduction = bool.Parse(ConfigurationManager.AppSettings["cybs.sendToProduction"].ToString());
        config.ServerURL = ConfigurationManager.AppSettings["cybs.serverURL"].ToString();
        //config.Timeout = int.Parse(ConfigurationManager.AppSettings["cybs.timeout"].ToString());
    }
    #endregion

    #region Authorize For Payment 
    ...
    private void HandleReply()
    {
        try
        {
                ...
                case 100:
                   ...
                   string REQUEST_TOKEN = reply.requestToken;
                    ...
                    if (!(String.IsNullOrEmpty(Request.QueryString.ToString())))
                    {
                        ...
                        //Insert Update To Pre Payment Table
                        objStoreFront.InsertAppointmentPrePayment(Convert.ToInt32(Session["UserID"]), RevenueCost, PriorRevenueCost, TipCost, "", Convert.ToInt32(Session["PayID"]), Convert.ToInt32(GroomerAppId), billTo_firstName.Text, billTo_lastname.Text);
                        //update Pre Pay Status In GroomerAppointment Table
                        objStoreFront.UpdatePrePayStatus(GroomerAppId, "PrePaid");
                        //Send Mail to User And Member
                        PaymentDoneMessage();
                        ...
                        ClearCreditCardDetails();
                        Response.Redirect("~/MyAccount.aspx?" + EncryptQueryString(string.Format("msg ={0}&Revenue ={1}&AdditionalRevenue={2}&Tip={3}", "S", RevenueCost, PriorRevenueCost, TipCost)), false);
                    }
            ...
            if (IsResponsefound)
            {
                ...
                lblerrormsg.Visible = true;
                Response.Redirect("~/MyAccount.aspx?" + EncryptQueryString(string.Format("msg ={0}&Revenue ={1}&AdditionalRevenue={2}&Tip={3}", "U", 0, 0, 0)), false);
            }
    ...
    protected void GrdFutureApp_RowCreated(object sender, GridViewRowEventArgs e)
    {
        ...
        //{
        //    GridViewRow gvr = e.Row;
        //    RadioButton rb = (RadioButton)gvr.Cells[0].FindControl("RadioButton1");
        //    AsyncPostBackTrigger trigger = new AsyncPostBackTrigger();
        //    trigger.ControlID = rb.CssClass;
        //    trigger.EventName = "CheckedChanged";
        //    payUpdate.Triggers.Add(trigger);
        //}
    ...
    //        Global1.AddShopperFields(request, ref shopper);
    //        Global1.AddBillAddressFields(request, ref billAddress);
    //        Global1.AddShipToAddressFields(request, ref shipToAddress);
                ...
                if (!(null == Session["IsExecuted"]))
                {
                    if (Session["IsExecuted"].Equals("0"))
                    {
                        ...
                        if (totalAmount > 0)
                        {
                                ...
                                //User Appointment First Insert/Update to Groomer Appointment 
                                ...
                                #region Insert/Update to shopper info before payment  and set PayId
    ...
    protected void RadioButton1_CheckedChanged(object sender, EventArgs e)
    {
        foreach (GridViewRow row in GrdFutureApp.Rows)
        {
            ...
            if (rb.Checked)
            {
                ...
                string PayStatus = ((Label)(GrdFutureApp.Rows[row.RowIndex].Cells[4].FindControl("lblPaymentStatus"))).Text;
    ...
    #region GetPreRevenueSetByAdminOrGroomer
    protected void GetPreRevenueSetByAdminOrGroomer(int AppId)
    {
        StoreFrontUser Obj = new StoreFrontUser();
        DataSet ds1 = new DataSet();
        ds1 = Obj.GetPaymentAppointmentByGrommerDetail(AppId);
        if (ds1.Tables.Count == 2)
        {
            ...
            {
                if (ds1.Tables[0].Rows[0]["StatusPresented"].ToString() == "True" && ds1.Tables[0].Rows[0]["Status"].ToString() == "Completed")
                {
                    ///check  revenue here 
                    if (ds1.Tables[1].Rows[0]["RevenueCreditCard"].ToString() != "0.0000")
                    {
                        txtRevenueCost.Text = ds1.Tables[1].Rows[0]["RevenueCreditCard"].ToString();
                        txtRevenueCost.Enabled = false;
                    }
                    if (ds1.Tables[1].Rows[0]["RevenueCheck"].ToString() != "0.0000")
                    {
                        txtRevenueCost.Text = ds1.Tables[1].Rows[0]["RevenueCheck"].ToString();
                        txtRevenueCost.Enabled = false;
                    }
                    if (ds1.Tables[1].Rows[0]["RevenueCash"].ToString() != "0.0000")
                    {
                        txtRevenueCost.Text = ds1.Tables[1].Rows[0]["RevenueCash"].ToString();
                        txtRevenueCost.Enabled = false;
                    }
                    if (ds1.Tables[1].Rows[0]["RevenueInvoice"].ToString() != "0.0000")
                    {
                        txtRevenueCost.Text = ds1.Tables[1].Rows[0]["RevenueInvoice"].ToString();
                        txtRevenueCost.Enabled = false;
                    }
                    if (ds1.Tables[1].Rows[0]["RevenueCCY"].ToString() != "0.0000")
                    {
                        txtRevenueCost.Text = ds1.Tables[1].Rows[0]["RevenueCCY"].ToString();
                        txtRevenueCost.Enabled = false;
                    }
                    ///check prior revenue here 
                    if (ds1.Tables[1].Rows[0]["PriorCreditCard"].ToString() != "0.0000")
                    {
                        txtPriorRevenue.Text = ds1.Tables[1].Rows[0]["PriorCreditCard"].ToString();
                        txtPriorRevenue.Enabled = false;
                    }
                    if (ds1.Tables[1].Rows[0]["PriorCheck"].ToString() != "0.0000")
                    {
                        txtPriorRevenue.Text = ds1.Tables[1].Rows[0]["PriorCheck"].ToString();
                        txtPriorRevenue.Enabled = false;
                    }
                    if (ds1.Tables[1].Rows[0]["PriorCash"].ToString() != "0.0000")
                    {
                        txtPriorRevenue.Text = ds1.Tables[1].Rows[0]["PriorCash"].ToString();
                        txtPriorRevenue.Enabled = false;
                    }

                    ///check tip ammount 
                    if (ds1.Tables[1].Rows[0]["TipCreditCard"].ToString() != "0.0000")
                    {
                        txtTipAmt.Text = ds1.Tables[1].Rows[0]["TipCreditCard"].ToString();
                        txtTipAmt.Enabled = true;
                    }
                    if (ds1.Tables[1].Rows[0]["TipCheck"].ToString() != "0.0000")
                    {
                        txtTipAmt.Text = ds1.Tables[1].Rows[0]["TipCheck"].ToString();
                        txtTipAmt.Enabled = true;
                    }
                    if (ds1.Tables[1].Rows[0]["TipCash"].ToString() != "0.0000")
                    {
                        txtTipAmt.Text = ds1.Tables[1].Rows[0]["TipCash"].ToString();
                        txtTipAmt.Enabled = true;
                    }
                    if (ds1.Tables[1].Rows[0]["TipInvoice"].ToString() != "0.0000")
                    {
                        txtTipAmt.Text = ds1.Tables[1].Rows[0]["TipInvoice"].ToString();
                        txtTipAmt.Enabled = true;
                    }
                ...
                else
                {
                    if (ds1.Tables[0].Rows.Count > 0)
                    {
                        txtRevenueCost.Text = ds1.Tables[0].Rows[0]["ExpectedTotalRevenue"].ToString();
                        txtRevenueCost.Enabled = false;
                        // txtPriorRevenue.Enabled = false;
                        txtPriorRevenue.Text = "0";
                        // txtTipAmt.Enabled = true;
                        txtTipAmt.Text = "0";
                        lblTotalAmt.Text = ds1.Tables[0].Rows[0]["ExpectedTotalRevenue"].ToString();
                    }
                }
            }
        }
    ...
    #region BindPayMentFieldsForPayment
